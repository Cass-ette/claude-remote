...
describe("loadConfig", () => {
  ...
  describe("BRIDGE_ENV_FILE", () => {
    function envFile(dir: string, contents: string, mode = 0o600): string {
    ...
    it("loads secret knobs from an owner-only env file (Task 34 launchd installer)", () => {
      const dir = dataDir();
      const file = envFile(dir, [
        "# generated by install-launchd (0600)",
      ...
      const cfg = loadConfig({
        ...
        BRIDGE_PORT: "43111",
        BRIDGE_DATA_DIR: dir,
        BRIDGE_ENV_FILE: file,
      });
      expect(cfg.cloudflareTeamDomain).toBe("myteam.cloudflareaccess.com");
      expect(cfg.cloudflareAud).toBe("0e9a5b2f7dbf4e1b9a17d8e0c3f2a1b0");
      expect(cfg.publicHost).toBe("bridge.example.com");
    });

    it("keeps real environment values above env-file values", () => {
      const dir = dataDir();
      const file = envFile(
        dir,
        ["BRIDGE_PORT=9999", "BRIDGE_CLOUDFLARE_AUD=file-aud", "BRIDGE_HOST=::1"].join("\n"),
      );
      const cfg = loadConfig({
        BRIDGE_HOST: "127.0.0.1",
        BRIDGE_PORT: "44444",
        BRIDGE_DATA_DIR: dir,
        BRIDGE_ENV_FILE: file,
        BRIDGE_CLOUDFLARE_AUD: "env-aud",
      });
      expect(cfg.port).toBe(44444);
      expect(cfg.host).toBe("127.0.0.1");
      expect(cfg.cloudflareAud).toBe("env-aud");
    });

    it("rejects missing, relative, or group/world-readable env files", () => {
      const dir = dataDir();
      expect(() =>
        loadConfig({ BRIDGE_HOST: "127.0.0.1", BRIDGE_PORT: "43111", BRIDGE_DATA_DIR: dir, BRIDGE_ENV_FILE: "relative.env" }),
      ...
      expect(() =>
        loadConfig({
          ...
          BRIDGE_PORT: "43111",
          BRIDGE_DATA_DIR: dir,
          BRIDGE_ENV_FILE: join(dir, "missing.env"),
        }),
      ).toThrow(/BRIDGE_ENV_FILE/);
      const leaky = envFile(dir, "BRIDGE_CLOUDFLARE_AUD=x\n", 0o644);
      expect(() =>
        loadConfig({ BRIDGE_HOST: "127.0.0.1", BRIDGE_PORT: "43111", BRIDGE_DATA_DIR: dir, BRIDGE_ENV_FILE: leaky }),
      ).toThrow(/BRIDGE_ENV_FILE/);
    });
  });
});
